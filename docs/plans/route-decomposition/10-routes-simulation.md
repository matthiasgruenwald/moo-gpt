# Schritt 10: routes/simulation.js ✅ erledigt

Simulation (Einzelpersona) und One-Click-Optimierung (alle Personas parallel).
Beide Endpunkte streamen via SSE.
...
|---------|------|------|-------------------|
| POST | /api/simulate | requireDashboardAuth | 704 |
| POST | /api/one-click-optimize | requireDashboardAuth | 818 |

---

## Mitziehende Hilfsfunktion: `selectPersonasForOneClick`

Aktuell in `server.js` (Zeilen 769–814), wird nur von `/api/one-click-optimize` genutzt.
Zieht in `routes/simulation.js` als private (nicht exportierte) Funktion.
...
import {
  ...
  getCriteria, getActiveErfahrungsprompt, saveErkenntnisse,
} from '../db.js';
...
- Zeilen 769–814 (`selectPersonasForOneClick` + `ONE_CLICK_FALLBACK_NAMES`)
- Zeilen 704–763 (`/api/simulate`)
- Zeilen 818–894 (`/api/one-click-optimize`)
...
# POST /api/simulate?token=... { personaId: 1, activityId: "TEST" }
# → SSE-Stream mit start, progress, pair(s), suggestion, done events
# POST /api/one-click-optimize?activityId=TEST&token=...
# → SSE-Stream mit criteria, personas, sim_start, sim_pair(s), optimize_done events
```
