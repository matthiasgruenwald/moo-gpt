# Schritt 3: routes/activity.js

Aktivitäts-Konfiguration und Plenum-Sperre.
...
|---------|------|------|-------------------|
| GET | /api/activity-config/:activityId | requireDashboardAuth | 239 |
| PUT | /api/activity-config/:activityId | requireDashboardAuth | 258 |
| POST | /api/activity/:activityId/lock | requireDashboardAuth | 666 |
| DELETE | /api/activity/:activityId/lock | requireDashboardAuth | 690 |

---

## Warum Factory statt Plain Router

Die Lock-Endpunkte brauchen `chatRegistry`, `dashboardRegistry` (für Broadcast) und
`activityLocks` (Map). Diese leben in `server.js` und werden auch vom WebSocket-Handler
und `ChatSession` genutzt — sie können nicht einfach in dieses Modul wandern.
...
**Achtung:** Die beiden Endpunkt-Pfade haben unterschiedliche URL-Wurzeln:
`/api/activity-config/:id` und `/api/activity/:id/lock`. Beide unter `/api` mounten.

---

## Entfernen aus server.js

Nach erfolgreichem Test Zeilen 239–266 und 666–699 aus `server.js` löschen,
sowie den temporären Import von `validateTemplateFields` falls nur noch hier gebraucht.
...
# GET /api/activity-config/TEST?token=... → JSON mit activityId
# POST /api/activity/TEST/lock?token=... → { ok: true, locked: true }
# Schüler-Widget öffnen → sollte "locked"-Event empfangen wenn Sperre aktiv
```
