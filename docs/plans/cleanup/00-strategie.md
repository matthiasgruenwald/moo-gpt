...
| 03 | `optimize.js` | Greift selbst in drei Stores. | Store-Zugriffe an Caller delegieren | ✓ done |
...
| 05 | `server.js` / `routes/activity.js` | Aktivitätssperren als rohes `Map` in `server.js`, direkt aus Route manipuliert. Timer und Broadcasts verteilt. | → `lock-manager.js` | ✓ done |
