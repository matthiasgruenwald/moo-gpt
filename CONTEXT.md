...
**Chat-Modell (`model`):** GPT-Modell für den Schüler-Chat. Priorität (volle Kaskade, aufgelöst in `resolveWidgetConfig`, siehe ADR 0008): `activities.model` → Lehrer-Vorlage → Systemvorlage → Admin-Systemprompt-Modell (`prompts.model`) → `MODEL_NAME` (Env). Jede Stufe wird gegen `AVAILABLE_MODELS` validiert; ungültige Werte fallen zur nächsten Stufe durch. Dropdown zeigt `AVAILABLE_MODELS`. UI: im `<details>`-Block „Erweitert" im Config-Overlay.
