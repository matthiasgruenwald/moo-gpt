...
console.log("ubeta chatbot script added");
...
export class UBETA {
  ...
  oracleQuery = "/talk/query";
  oracleStream = "/talk/chat";
  ...
  constructor(settings) {
    console.log("constructing ubeta");
   ...
   */
  renderInit() {
    console.log("initializing ubeta chatbot");
    ...
    this.root.innerHTML = `
            <div class="ubeta-chat-window" ${this.startOpen ? "" : "hidden"}>
                ...
                <div class="ubeta-input-container">
                    <input class="ubeta-console" type="text" placeholder="${
    ...
    this.root.querySelector(".ubeta-window-handle").addEventListener(
      "mousedown",
      (e) => {
  ...
  askOracle(message) {
    ...
    if (this.streamResponse) {
      this.sendOracleStream(body);
    } else {
      this.sendOracleQuery(body);
    }
  }

  sendOracleQuery(body) {
    fetch(this.apiPath + this.oracleQuery, {
      method: "POST",
      headers: {
      ...
      .catch(
        ((err) => {
          console.log(err);
          this.addErrorChat(
            "Sorry, there seems to be something wrong with my network connection.  Please try again later."
          );
        }).bind(this)
      );
  }

  sendOracleStream(body) {
    const ctrl = new AbortController();
    fetchEventSource(this.apiPath + this.oracleStream, {
      method: "POST",
      headers: {
        Referer: window.location.href,
        "Content-Type": "application/json",
      ...
      onmessage: (event) => {
        if (event.data === "[DONE]") {
          ...
          this.updateAssistantStream(data);
        }
      },
  ...
  getTranscript() {
    ...
    this.root.querySelectorAll(".ubeta-message").forEach((message) => {
      ...
      transcript.push([
        message.getAttribute("aria-label") + ": " + message.innerText,
      ]);
    });
    ...
    if (this.showSource && data.sourceDocuments.length > 0) {
      ...
      sources = `
            <div class='ubeta-sources-container'>
                ...
                <ul id='content-${id}' class='ubeta-sources' hidden>${data.sourceDocuments
        .map((source) => {
  ...
  startAssistantStream() {
    ...
    log.appendChild(
      new DOMParser().parseFromString(
        `
            <div class='ubeta-message ubeta-assistant' aria-label='assistant' hidden>
                <div class='ubeta-text'></div>
            </div>
        `,
        "text/html"
      ).body.firstElementChild
  ...
  updateAssistantStream(data) {
    var lastList = this.root.querySelectorAll(
      ".ubeta-chat-log .ubeta-message.ubeta-assistant"
    );
    var lastLog = lastList[lastList.length - 1];
    if (this.showSource && data.sourceDocs) {
      var id = window.crypto.randomUUID();
      var sources = `
            <div class='ubeta-sources-container'>
                <button id='button-${id}' class='ubeta-sources-button'>
                ...
                <ul id='content-${id}' class='ubeta-sources' hidden>${data.sourceDocs
        .map((source) => {
          return `<li>${source.pageContent}</li>`;
        })
        .join("")}</ul>
      ...
      lastLog.querySelector(".ubeta-text").innerHTML += data.data;
      if (data.data.length > 0 && lastLog.hasAttribute("hidden")) {
        lastLog.removeAttribute("hidden");
        this.root.querySelector(".ubeta-ellipsis").setAttribute("hidden", true);
      }
    }
